...
if (!$articles_by_category || count($articles_by_category) == 0) {
  ...
  for ($i = 0; $i < count($articles_by_category[0]); $i++) {
    ...
    $single_article .= '<img tabindex="0" src="img/banner_img/' . $articles_by_category[2][$i]->id .'.jpg" alt="' . $articles_by_category[2][$i]->alt_text . '" class="content-section-1-banner">';
    $single_article .= '</div>';
    $single_article .= '<div class="col-md-6 align-middle">';
    $single_article .= '<a href="content-page.php?article=' . $articles_by_category[0][$i]->id . '" target="_self"><p class="font-bold">' . $articles_by_category[0][$i]->title . '</p></a>';
